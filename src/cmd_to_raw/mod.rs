...
#[derive(Clone)]
pub struct RingBufferIndexState {
    total_offset: usize,
...
pub struct DivansRecodeState<RingBuffer: SliceWrapperMut<u8> + SliceWrapper<u8>>{
    ...
    state: RingBufferIndexState,
}

const REPEAT_BUFFER_MAX_SIZE: u32 = 64;
pub type RingBufferSnapshot = RingBufferIndexState;
...
impl<RingBuffer: SliceWrapperMut<u8> + SliceWrapper<u8>> DivansRecodeState<RingBuffer> {
    ...
    pub fn snapshot_ringbuffer(&self) -> RingBufferSnapshot {
        self.state.clone()
    }
    pub fn restore_ringbuffer_to_snapshot(&mut self, data: RingBufferSnapshot) {
        self.state = data;
    }
    #[inline(always)]
    ...
    pub fn last_8_literals(&self) -> [u8; 8] {
        if self.state.ring_buffer_decode_index < 8 {
            self.fallback_last_8_literals()
        } else {
            let mut ret = [0u8; 8];
            ret.clone_from_slice(self.ring_buffer.slice().split_at(self.state.ring_buffer_decode_index as usize - 8).1.split_at(8).0);
            ret
        }
    }
    ...
    pub fn flush(&mut self, output :&mut[u8], output_offset: &mut usize) -> DivansOutputResult {
        ...
        if self.state.ring_buffer_decode_index < self.state.ring_buffer_output_index { // we wrap around
            let bytes_until_wrap = self.ring_buffer.slice().len() - self.state.ring_buffer_output_index as usize;
            ...
            output[*output_offset..(*output_offset + amount_to_copy)].clone_from_slice(
                &self.ring_buffer.slice()[self.state.ring_buffer_output_index as usize..(self.state.ring_buffer_output_index as usize
                                                                         + amount_to_copy)]);
            self.state.ring_buffer_output_index += amount_to_copy as u32;
            *output_offset += amount_to_copy;
            if self.state.ring_buffer_output_index as usize == self.ring_buffer.slice().len() {
               self.state.ring_buffer_output_index = 0;
            }
        }
        if *output_offset != output.len() && self.state.ring_buffer_output_index < self.state.ring_buffer_decode_index {
            let amount_to_copy = core::cmp::min((self.state.ring_buffer_decode_index - self.state.ring_buffer_output_index) as usize ,
                                                output.len() - *output_offset);
            
            output.split_at_mut(*output_offset).1.split_at_mut(amount_to_copy).0.clone_from_slice(
                &self.ring_buffer.slice().split_at(self.state.ring_buffer_output_index as usize).1.split_at(amount_to_copy).0);
            self.state.ring_buffer_output_index += amount_to_copy as u32;
            *output_offset += amount_to_copy;
            if self.state.ring_buffer_output_index as usize == self.ring_buffer.slice().len() {
               self.state.ring_buffer_output_index = 0;
        ...
        self.state.total_offset += *output_offset - prev_output_offset;
        if self.state.ring_buffer_output_index != self.state.ring_buffer_decode_index {
    ...
    fn decode_space_left_in_ring_buffer(&self) -> u32 {
        ...
        if self.state.ring_buffer_output_index <= self.state.ring_buffer_decode_index {
            return self.state.ring_buffer_output_index + self.ring_buffer.slice().len() as u32 - 1 - self.state.ring_buffer_decode_index;
        }
        self.state.ring_buffer_output_index - 1 - self.state.ring_buffer_decode_index
    }
    fn copy_decoded_from_ring_buffer(&self, mut output: &mut[u8], mut distance: u32, mut amount_to_copy: u32) {
        if distance > self.state.ring_buffer_decode_index {
            // we need to copy this in two segments...starting with the segment far past the end
            let far_distance = distance - self.state.ring_buffer_decode_index;
            ...
            output = output_near;
            distance = self.state.ring_buffer_decode_index;
            amount_to_copy -= far_amount as u32;
        }
        if !output.is_empty() {
            let start = self.state.ring_buffer_decode_index - distance;
    ...
    fn copy_some_decoded_from_ring_buffer_to_decoded<CopyCallback:FnMut(&[u8])>(
        ...
        if self.state.ring_buffer_decode_index as u32 >= distance {
            src_distance_index = self.state.ring_buffer_decode_index - distance;
        } else {
            src_distance_index = self.state.ring_buffer_decode_index + self.ring_buffer.slice().len() as u32;
        ...
        let mut trunc_amount_to_copy = core::cmp::min(core::cmp::min(left_dst_before_wrap,
                                                  ...
                                                  desired_amount_to_copy);
        if src_distance_index < self.state.ring_buffer_decode_index {
            let (_unused, src_and_dst) = self.ring_buffer.slice_mut().split_at_mut(src_distance_index as usize);
            let (src, dst) = src_and_dst.split_at_mut((self.state.ring_buffer_decode_index - src_distance_index) as usize);
            dst.split_at_mut(trunc_amount_to_copy as usize).0.clone_from_slice(src.split_at_mut(trunc_amount_to_copy as usize).0);
        } else {
            let (_unused, dst_and_src) = self.ring_buffer.slice_mut().split_at_mut(self.state.ring_buffer_decode_index as usize);
            let (dst, src) = dst_and_src.split_at_mut((src_distance_index - self.state.ring_buffer_decode_index) as usize);
    ...
    fn copy_to_ring_buffer(&mut self, mut data: &[u8]) -> usize {
        ...
        let first_section = self.ring_buffer.slice_mut().len() as u32 - self.state.ring_buffer_decode_index;
        ...
        self.ring_buffer.slice_mut().split_at_mut(self.state.ring_buffer_decode_index as usize).1.split_at_mut(amount_to_copy as usize).0.clone_from_slice(data_first);
        self.state.ring_buffer_decode_index += amount_to_copy as u32;
        retval += amount_to_copy as usize;
        if self.state.ring_buffer_decode_index == self.ring_buffer.slice().len() as u32 {
            self.state.ring_buffer_decode_index = 0;
            let second_amount_to_copy = data_second.len();
            self.ring_buffer.slice_mut().split_at_mut(self.state.ring_buffer_decode_index as usize).1.split_at_mut(second_amount_to_copy).0.clone_from_slice(data_second.split_at(second_amount_to_copy).0);
            self.state.ring_buffer_decode_index += second_amount_to_copy as u32;
    ...
    fn parse_literal(&mut self, data:&[u8]) -> DivansOutputResult {
       let data_len = data.len(); 
        if data_len < self.state.input_sub_offset { // this means user passed us different data a second time
           return DivansOutputResult::Failure(ErrMsg::InputChangedAfterContinuation);
       }
       let remainder = data.split_at(self.state.input_sub_offset).1;
       let bytes_copied = self.copy_to_ring_buffer(remainder);
       self.state.input_sub_offset += bytes_copied as usize;
    ...
    fn parse_copy_simplified(&mut self, copy:&CopyCommand) -> DivansOutputResult {
        for i in (self.state.input_sub_offset as usize)..(copy.num_bytes as usize){
            if ((self.state.ring_buffer_decode_index + 1) & (self.ring_buffer.slice().len() as u32 - 1)) == self.state.ring_buffer_output_index {
               self.state.input_sub_offset = i;
               return DivansOutputResult::NeedsMoreOutput;
            }
            let mut src = self.state.ring_buffer_decode_index + self.ring_buffer.slice().len() as u32 - copy.distance;
            src &= self.ring_buffer.slice().len() as u32 - 1;
            let src_val = self.ring_buffer.slice()[src as usize];
            self.ring_buffer.slice_mut()[self.state.ring_buffer_decode_index as usize] = src_val;
            self.state.ring_buffer_decode_index += 1;
            if self.state.ring_buffer_decode_index == self.ring_buffer.slice().len() as u32 {
               self.state.ring_buffer_decode_index = 0;
            }
        }
        self.state.input_sub_offset = copy.num_bytes as usize;
        DivansOutputResult::Success
    }
    #[inline(always)]
    fn parse_copy<CopyCallback:FnMut(&[u8])>(
        ...
        self.state.input_sub_offset += copy_count as usize;
    ...
    pub fn encode_cmd<SliceType:SliceWrapper<u8>, CopyCallback:FnMut(&[u8])>(
        ...
        if ((self.state.ring_buffer_decode_index as u32 ^ self.state.ring_buffer_output_index as u32) & 0xffff_fc00) == 0 {
            self.state.input_sub_offset = 0;
            return DivansOutputResult::Success;
        }
        match self.flush(output, output_offset)  {
            DivansOutputResult::Success => {
